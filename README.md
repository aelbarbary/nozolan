...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv nozolan` or `python -m venv nozolan-venv`.
...
- [ ] Make sure the virtualenv is activated `workon nozolan` or `source nozolan-venv/bin/activate`.
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `nozolan/backend/nozolan/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `nozolan/backend/nozolan/settings/production.py`
...
- Create a copy of ``nozolan/settings/local.py.example``:  
  `cp nozolan/settings/local.py.example nozolan/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/nozolan/settings/base.py``.
- Adjust ``backend/nozolan/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
